...
function wo_update_client( $client = null ) {
	...
	$client_data = array(
		'ID'         => sanitize_text_field( $client['edit_client'] ),
		'post_title' => sanitize_text_field( $client['name'] )
	);
	...
	update_post_meta( $client_data['ID'], 'client_id', sanitize_text_field( $client['client_id'] ) );
	update_post_meta( $client_data['ID'], 'client_secret', sanitize_text_field( $client['client_secret'] ) );
	update_post_meta( $client_data['ID'], 'grant_types', $grant_types );
	update_post_meta( $client_data['ID'], 'redirect_uri', esc_url( $client['redirect_uri'] ) );
	update_post_meta( $client_data['ID'], 'user_id', intval( $client['user_id'] ) );
	update_post_meta( $client_data['ID'], 'scope', sanitize_text_field( $client['scope'] ) );
}
